  ...
  persist(
    (set, get) => ({
      ...
      sendScout: (biomeId, animalType) => {
        ...
        const now = Date.now();
        const mission: ScoutMission = { animalType, departureTime: now, returnTime: now + durationHours * 3600000, durationHours };
        set(state => ({ biomes: { ...state.biomes, [biomeId]: { ...state.biomes[biomeId], status: 'scouting' as const, mission } } }));
      },
      checkReturnedScouts: () => {
        const now = Date.now();
        ...
        for (const id of Object.keys(updated) as BiomeId[]) {
          ...
          if (b.status === 'scouting' && b.mission && now >= b.mission.returnTime) {
            updated[id] = { ...b, status: 'scout_returned', report: BIOME_CONFIGS[id].scoutReport };
            returned.push(id);
            changed = true;
          }
        }
      ...
      acknowledgeReport: (biomeId) => {
        set(state => ({ biomes: { ...state.biomes, [biomeId]: { ...state.biomes[biomeId], status: 'unlocking' as const } } }));
      },
      addSteps: (steps) => {
        const state = get();
        for (const id of Object.keys(state.biomes) as BiomeId[]) {
          ...
          if (b.status === 'unlocking' && b.report) {
            ...
            set(prev => ({ biomes: { ...prev.biomes, [id]: { ...prev.biomes[id], stepsCompleted: Math.min(newSteps, b.report!.stepsRequired), status: justUnlocked ? 'unlocked' as const : 'unlocking' as const } } }));
